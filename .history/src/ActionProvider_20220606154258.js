  ...
  async greet(answer) {
    try {
      ...
      if (mensaje.includes("la potencia la derivada del trabajo respecto del tiempo")){
        ...
        var cirigido = (mess.includes("cir") || mess.includes("cm") || mess.includes("centro de masa") || mess.includes("rígido") || mess.includes("rigido"))
        if ( cinematica && not (cirigido)){
            const Message = this.createChatbotMessage("Revisá estos links", {
            widget: "cinelinks"});
            this.updateChatbotState(Message);
        }
        else if (cinrigido && not (mess.includes("partículas"))){
          const Message = this.createChatbotMessage("Revisá estos links", {
